...
public class ScoreDataRedisCache implements ScoreDataCache {
    ...
    @Override
    public void cache(ReportType reportType, User user, Certificate certificate,
            ScoreDataList scoreData) {
        String key = getKey(reportType, user, certificate);
        redisTemplate.opsForValue().setIfAbsent(key, scoreData);
    }

    @Override
    public Optional<ScoreDataList> get(ReportType reportType, User user, Certificate certificate) {
        String key = getKey(reportType, user, certificate);
    ...
    private String getKey(ReportType reportType, User user, Certificate certificate) {
        return String.format("SCORE_DATA:%s:%s:%s", reportType, user.getId(), certificate.id().value());
    }
